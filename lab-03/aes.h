#ifndef LAB_03_AES_H
#define LAB_03_AES_H

#include<map>
#include<vector>
#include <bitset>
#include <bit>
#include <cstdint>
#include <algorithm>
#include <utility>

using namespace std;

typedef vector<uint8_t> key;
typedef vector<uint8_t> block;
typedef vector<vector<uint8_t>> mtx;

enum AES_MODE {
    AES128,
    AES192,
    AES256
};

class IEncoder {

public:
    virtual block EncryptBlock(const block& block, const key& key) = 0;
    virtual block DecryptBlock(const block& block, const key& key) = 0;
};

// AES - 128 бит
class aes: public IEncoder{
public:
    block CryptBlock(const block& block128, const key& key128, bool decrypter = false);
    block EncryptBlock(const block& block128, const key& key128) override;
    block DecryptBlock(const block& block128, const key& key128) override;

    vector<key> GetKeys128(key key128, bool decrypter = false);
    void AddRoundKey(mtx& block, const key& roundKey);
    void SubBytes(mtx& block);
    void InvSubBytes(mtx& block);
    void ShiftRows(mtx& block);
    void InvShiftRows(mtx& block);
    void MixColumns(mtx& block);
    void InvMixColumns(mtx& block);

    mtx ArrayToMrx4x4(const block& block);
    block Mrx4x4ToArray(const mtx &mtx);
    void AddPadding(vector<uint8_t>& data);
    void RemovePadding(vector<uint8_t>& data);
private:
    // Умножение поля Галуа
    uint8_t GMul(uint8_t x, uint8_t y);

    AES_MODE mode = AES128;

    unsigned int AmountPadding = 0;

    // Расширение ключа
    // vector<key> key_expasion(key key128);
    // Функция замены байтов через S-Box
    key sub_word(key& word);
    // Функция циклического сдвига байтов влево на один байт
    key rot_word(key& word);

    // Ключ по умолчанию, если нигде не был найден - 128 бит или 16 байт ~ 16 символам.
    vector<uint8_t> default_key = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f};

    // 11 --- AES-28, 13 --- AES-192, 16 --- AES-156
    const uint8_t R_CON[16] = { 0x8d,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36,0x6c,0xd8,0xab,0x4d,0x9a };

    // S-box
    const uint8_t S_BOX[256] = {
            
    };

    const uint8_t  S_BOX_INV[256] = {
            
    };

    //
};


#endif //LAB_03_AES_H
